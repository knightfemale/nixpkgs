...
- `wivrn` has dropped support for PulseAudio in 26.9. WiVRn now only supports audio through PipeWire.
...
- TUI command of `tracexec` now allocates a pseudo terminal by default. Use `--no-tty` to run without one and redirect the tracee's stdin, stdout, and
  stderr to `/dev/null`. eBPF system-wide tracing continues to run without a pseudo terminal automatically. The old `-t/--tty` parameter is removed.
...
- `openbao` has been updated to 2.7.0, which no longer includes the `pkcs11`, `alicloudkms`, `awskms`, `azurekeyvault`, `gcpckms` and `ocikms` seals, the LDAP, Kerberos and RADIUS auth methods, or the LDAP secrets engine. The seals are available as plugins in `pkgs.openbaoPlugins` and can be installed with [`services.openbao.plugins`](#opt-services.openbao.plugins). The `withHsm` override argument has been removed.
...
- The `programs.dms-shell.enableVPN`, `enableDynamicTheming`, `enableAudioWavelength`, and `enableCalendarEvents` options have been removed. `matugen` and `cava` are now included in the default DMS environment and can be excluded with `programs.dms-shell.excludePackages`. Networking and calendar backends are configured separately.
- `programs.dms-shell` no longer enables `services.power-profiles-daemon` by default. Configure a power profile provider separately.
...
- The latest available version of Nextcloud is v34 (available as `pkgs.nextcloud34`). The installation logic is as follows:
  ...
  - `nextcloud32` is EOL and was thus removed.
  - Please note that an upgrade from v32 (or older) to v34 directly is not possible. Please upgrade to `nextcloud33` (or earlier) first. Nextcloud prohibits skipping major versions while upgrading. You can upgrade by declaring `services.nextcloud.package = pkgs.nextcloud33;`.
